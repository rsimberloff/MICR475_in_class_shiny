...
# Designing User Interface of Shiny App
ui <- fluidPage(
                titlePanel("Kernal Density Home Range of a Specific Sparrow"), #Adding title panel
                sliderInput(inputId = "h", #Adding slider scale based on user input h
                            label = "Kernal Width (h)",
                            min = 0.00005, #Setting max and min kernel width values
                            max = 0.001,
                            value = 0),
                plotOutput(outputId = "densityplot") #Setting output to plot
